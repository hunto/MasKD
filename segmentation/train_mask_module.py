...
from losses.maskd import MaskModule
...
def parse_args():
    ...
    parser.add_argument('--max-iterations', type=int, default=10000, metavar='N',
                        help='number of epochs to train (default: 50)')
    parser.add_argument('--lr', type=float, default=0.1, metavar='LR',
                        help='learning rate (default: 1e-4)')
    parser.add_argument('--momentum', type=float, default=0.9, metavar='M',
                        help='momentum (default: 0.9)')
    parser.add_argument('--weight-decay', type=float, default=1e-3, metavar='M',
...
class Trainer(object):
    def __init__(self, args):
        ...
        self.mask_module = MaskModule(channels=19, num_tokens=8)
        self.mask_module.cuda()
        # optimizer, for model just includes pretrained, head and auxlayer
        # params_list = self.model.parameters()
        params_list = self.mask_module.parameters()
        ...
        if args.distributed:
            ...
            self.mask_module = nn.parallel.DistributedDataParallel(self.mask_module, device_ids=[args.local_rank],
                                                                   output_device=args.local_rank, find_unused_parameters=True)
    ...
    def train(self):
        ...
        for iteration,  (images, targets, _) in enumerate(self.train_loader):
            ...
            # mask
            outputs[0], mask_loss = self.mask_module(outputs[0])
            ...
            if iteration % log_per_iters == 0 and save_to_disk:
                logger.info(
                    "Iters: {:d}/{:d} || Lr: {:.6f} || Task Loss: {:.4f} || Mask Loss: {:.4f} || Cost Time: {} || Estimated Time: {}".format(
                        iteration, args.max_iterations, self.optimizer.param_groups[0]['lr'], task_loss.item(), mask_loss.item(),
                        str(datetime.timedelta(seconds=int(time.time() - start_time))), eta_string))

            if iteration % save_per_iters == 0 and save_to_disk:
                save_checkpoint(self.mask_module, self.args, is_best=False)
    ...
    def validation(self):
        ...
        if args.local_rank == 0:
            save_checkpoint(self.mask_module, self.args, is_best)
        synchronize()
